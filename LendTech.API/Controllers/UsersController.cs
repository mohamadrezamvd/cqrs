...
public class UsersController : ControllerBase
{
	...
	public async Task<IActionResult> ChangePassword(Guid id, [FromBody] ChangePasswordDto request)
	{
		...
		if (currentUserId == null || (Guid.Parse(currentUserId) != id && !User.HasClaim(SecurityConstants.Claims.Permissions, "Users.ChangeOthersPassword")))
			return Forbid();
		...
		if (!result.IsSuccess)
		{
			if (result.Status == Ardalis.Result.ResultStatus.Unauthorized)
				return BadRequest(ApiResponse.Error(SharedKernel.Enums.ResponseStatus.Forbidden, result.Errors.FirstOrDefault() ?? "عدم دسترسی"));
			if (result.Status == Ardalis.Result.ResultStatus.NotFound)
				return NotFound(ApiResponse.NotFound("کاربر یافت نشد"));
			return BadRequest(ApiResponse.Error(SharedKernel.Enums.ResponseStatus.BusinessError, result.Errors.FirstOrDefault() ?? "خطا در تغییر رمز عبور"));
		}
